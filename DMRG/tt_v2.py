...
def tto(inp,     #[batch,784]  
       ...
       out_modes,           
       mat_ranks,
       cores_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")),
       cores_regularizer=tf.keras.regularizers.l2(0.5 * (0.0001)),
    ...
    with tf.compat.v1.variable_scope(scope):
        ...
        out = tf.transpose(a=out, perm=[1, 0]) # inp((n0,n1,...,nL-1),b)
        ...
        for i in range(dim):
            ...
            out = tf.transpose(a=out, perm=[1, 0])        
